...
export default function ConsultaScreen() {
  ...
  const campo = (
    ...
    <label style={css("display:flex;flex-direction:column;gap:7px;min-width:0;")}>
      <span style={css("font-size:10px;letter-spacing:.2em;text-transform:uppercase;color:var(--text-3);")}>{label}</span>
      <input
        size={4}
        value={f[key]}
        ...
        style={css(
          "width:100%;min-width:0;background:rgba(255,255,255,.9);border:1px solid var(--border-accent);border-radius:var(--r-sm);padding:12px 14px;color:var(--text);font-family:var(--font-ui);font-size:20px;transition:border-color .25s,box-shadow .25s;"
        )}
      />
    </label>
  );
  ...
  return (
    <main style={css("max-width:1120px;margin:0 auto;padding:clamp(26px,5vw,48px) clamp(14px,3vw,28px) 80px;animation:es33-in .5s ease both;")}>
      <div style={css("text-align:center;margin-bottom:40px;")}>
        <div style={css("font-size:11px;letter-spacing:.32em;text-transform:uppercase;color:var(--text-3);margin-bottom:14px;")}>Nueva consulta</div>
        <h1 style={css("font-family:var(--font-ui);font-weight:700;font-size:clamp(25px,4.6vw,38px);line-height:1.16;letter-spacing:.03em;color:var(--text);margin:0 0 12px;")}>
      ...
      <div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr));gap:26px;align-items:start;")}>
        ...
        >
          ...
          <div style={css("font-family:var(--font-ui);font-weight:600;font-size:11px;letter-spacing:.3em;color:var(--gold);text-transform:uppercase;margin-bottom:22px;")}>Datos de nacimiento</div>
          ...
          <button
            ...
            disabled={!listo}
            style={css(
              "width:100%;margin-top:26px;padding:14px 0;border-radius:var(--r-sm);cursor:" +
                (listo ? "pointer" : "not-allowed") +
                ";font-family:var(--font-ui);font-weight:600;font-size:12px;letter-spacing:.28em;text-transform:uppercase;border:1px solid " +
        ...
        <aside style={css("display:flex;flex-direction:column;gap:18px;")}>
          <div style={css("background:var(--surface);backdrop-filter:var(--blur);-webkit-backdrop-filter:var(--blur);box-shadow:var(--shadow);border:1px solid var(--border);border-radius:var(--r);padding:clamp(16px,2.2vw,22px) clamp(16px,2.4vw,24px);")}>
            <div style={css("font-family:var(--font-ui);font-weight:600;font-size:11px;letter-spacing:.28em;color:var(--gold);text-transform:uppercase;margin-bottom:8px;")}>Valor del nombre</div>
            <div style={css("display:flex;flex-wrap:wrap;gap:14px;margin:16px 0 6px;")}>
              {n.palabras.map((w, wi) => (
                <div key={wi} style={css("display:flex;flex-direction:column;gap:7px;")}>
                  ...
                  <div style={css("font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:var(--text-4);")}>
                    {w.palabra} · {w.total}
                  </div>
                </div>
              ))}
            ...
            <div style={css("display:flex;align-items:baseline;gap:10px;border-top:1px solid var(--border);margin-top:14px;padding-top:14px;")}>
              <span style={css("font-size:10px;letter-spacing:.2em;text-transform:uppercase;color:var(--text-3);")}>Número de corazón</span>
            ...
            <div style={css("display:flex;gap:18px;margin-top:10px;")}>
              <div style={css("display:flex;flex-direction:column;gap:2px;")}>
                <span style={css("font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:var(--text-4);")}>Esencia</span>
                <span style={css("font-family:var(--font-ui);font-weight:600;font-size:17px;color:var(--text-3);")}>{n.esencia || 0}</span>
              </div>
              <div style={css("display:flex;flex-direction:column;gap:2px;")}>
                <span style={css("font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:var(--text-4);")}>Ego</span>
          ...
          <div style={css("background:var(--surface);backdrop-filter:var(--blur);-webkit-backdrop-filter:var(--blur);box-shadow:var(--shadow);border:1px solid var(--border);border-radius:var(--r);padding:clamp(16px,2.2vw,22px) clamp(16px,2.4vw,24px);")}>
            <div style={css("font-family:var(--font-ui);font-weight:600;font-size:11px;letter-spacing:.28em;color:var(--gold);text-transform:uppercase;margin-bottom:14px;")}>Estudios guardados</div>
              ...
              {hist.map((h) => (
                <div key={h.id} style={css("display:flex;align-items:center;gap:10px;padding:9px 11px;border:1px solid var(--border);border-radius:var(--r-sm);background:rgba(0,0,0,.025);")}>
                  <button onClick={() => abrir(h)} style={css("flex:1;text-align:left;background:none;border:none;padding:0;cursor:pointer;color:var(--text);")}>
                    ...
                    <div style={css("font-size:10px;letter-spacing:.12em;color:var(--text-4);margin-top:2px;")}>
                      {h.fecha} · corazón {h.corazon}
                    </div>
                  </button>
                  <button onClick={() => borrar(h)} title="Eliminar" style={css("background:none;border:1px solid var(--red-border);color:var(--red);border-radius:var(--r-sm);width:24px;height:24px;cursor:pointer;font-size:13px;line-height:1;")}>
                    ×
                  </button>
                </div>
              ))}
